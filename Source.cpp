...
LRESULT CALLBACK windowProcedure(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp)
{
	...
	{
	case WM_CREATE:
		render.addObject(new Model2D(triangleLoader.vertices(), triangleLoader.indices()));
		render.addObject(new Model2D(trapezoidLoader.vertices(), trapezoidLoader.indices()));
		break;
	case WM_DESTROY:
		PostQuitMessage(0);
		break;
